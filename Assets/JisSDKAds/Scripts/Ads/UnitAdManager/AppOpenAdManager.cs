...
namespace SDK.AdsManagers
{
    public class AppOpenAdManager : UnitAdManager
    {
        ...
        {
            {
                ConfigValue configValue = FirebaseManager.Instance.GetConfigValue(Keys.key_remote_aoa_active);
                IsActiveByRemoteConfig = configValue.BooleanValue;
                DebugAds.Log("App Open Ads Active = " + IsActiveByRemoteConfig);
            }

            {
                ConfigValue configValue = FirebaseManager.Instance.GetConfigValue(Keys.key_remote_aoa_show_first_time_active);
