...
**Không bắt buộc sửa `manifest.json`:** dùng **Package Manager → + → Add package from git URL** (cùng URL bên dưới), rồi **JIS SDK → Hub**. OpenUPM chưa có package `com.jis.sdkads.*` — chỉ Git UPM + Hub.

Quick start (manifest hoặc Package Manager Git URL):
